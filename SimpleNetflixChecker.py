...
class SimpleNetflixChecker:
    ...
    def ui(self):
        os.system("cls && title Simple Netflix Checker by syedbilalalam")
        text = """ ░▄▀▀░█░█▄▒▄█▒█▀▄░█▒░▒██▀░█▄░█▒██▀░▀█▀▒█▀░█▒░░█░▀▄▀░▄▀▀░█▄█▒██▀░▄▀▀░█▄▀▒██▀▒█▀▄
▒▄██░█░█▒▀▒█░█▀▒▒█▄▄░█▄▄░█▒▀█░█▄▄░▒█▒░█▀▒█▄▄░█░█▒█░▀▄▄▒█▒█░█▄▄░▀▄▄░█▒█░█▄▄░█▀▄ """
        faded = ""
        red = 40
    ...
    def updateTitle(self):
        while True:
            ...
            os.system(
                f"title [Simple Netflix Checker] - Hits: {self.hits} ^| Bad: {self.bad} ^| Retries: {self.retries} ^| CPM: {self.cpm} ^| Threads: {threading.active_count() - 3} ^| Time elapsed: {elapsed}"
            )
            time.sleep(0.4)
    ...
    def getProxies(self):
        try:
            ...
            path = easygui.fileopenbox(
                default="*.txt",
                filetypes=["*.txt"],
                title="Simple Netflix Checker - Select proxy",
                multiple=False,
            )
            try:
    ...
    def getCombos(self):
        try:
            ...
            path = easygui.fileopenbox(
                default="*.txt",
                filetypes=["*.txt"],
                title="Simple Netflix Checker - Select combos",
                multiple=False,
            )
            with open(path, "r", encoding="utf-8") as f:
                for l in f:
